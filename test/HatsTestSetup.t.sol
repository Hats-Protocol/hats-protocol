...
abstract contract TestSetupNest is TestSetup {
    uint256 treeA;
    uint256 treeB;
    uint256 treeC;
    uint256 treeD;
    ...
    function setUp() public override {
        ...
        treeA = hats.mintTopHat(topHatWearer, "treeA", "http://www.tophat.com/");
        treeB = hats.mintTopHat(topHatWearer, "treeB", "http://www.tophat.com/");
        treeC = hats.mintTopHat(topHatWearer, "treeC", "http://www.tophat.com/");
        treeD = hats.mintTopHat(topHatWearer, "treeD", "http://www.tophat.com/");
        treeE = hats.mintTopHat(topHatWearer, "treeE", "http://www.tophat.com/");

        // derive domains for each
        domainA = uint32(treeA >> 224);
        domainB = uint32(treeB >> 224);
        domainC = uint32(treeC >> 224);
        domainD = uint32(treeD >> 224);
        domainE = uint32(treeE >> 224);

        // nest 5 of them under the tippy tophat
        hats.requestLinkTopHatToTree(domainA, topHatId);
        hats.approveLinkTopHatToTree(domainA, topHatId, address(0), address(0), "", "");
        hats.requestLinkTopHatToTree(domainB, treeA);
        hats.approveLinkTopHatToTree(domainB, treeA, address(0), address(0), "", "");
        hats.requestLinkTopHatToTree(domainC, treeB);
        hats.approveLinkTopHatToTree(domainC, treeB, address(0), address(0), "", "");
        hats.requestLinkTopHatToTree(domainD, treeC);
        hats.approveLinkTopHatToTree(domainD, treeC, address(0), address(0), "", "");
        hats.requestLinkTopHatToTree(domainE, treeD);
        hats.approveLinkTopHatToTree(domainE, treeD, address(0), address(0), "", "");
    }
}
